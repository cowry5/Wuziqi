...
//服务器端的鼠标抬起，就向客户端发送数据报，以便在客户端显示相应的结点
void CChessBorad::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	{
		NODE* node = GetLikeNode(pt); //获取鼠标所点击的位置
		if (node !=NULL)
		{
			if (node->m_Color==ncUNKOWN)
			{
				...
				//定义一个TCP数据报
				TCP_PACKAGE	 package;
				package.cmdType = CT_POINT;//记录数据报的命令类型
				package.chessPT.x = node->m_nX;//记录结点的横坐标
				package.chessPT.y = node->m_nY;//记录结点的纵坐标
				m_ClientSock.Send((void*)&package,sizeof(TCP_PACKAGE));//服务器向客户端发送数据报
...
void CChessBorad::BeginGame()
{
	...
	if (m_IsConnect)	//客户端当前必须与之建立连接
	{
		if (m_State!=esBEGIN)	//游戏已结束或未开始
		{
			...
			tcpPackage.cmdType = CT_BEGINGAME;//数据报发送的命令类型
			Start();//初始化信息
			//获取服务器昵称
			CString csNickName;
			...
			if (csNickName.IsEmpty())
				csNickName = "匿名";
			strncpy(tcpPackage.chText,csNickName,512);//数据报发送的文本文字
			m_ClientSock.Send(&tcpPackage,sizeof(TCP_PACKAGE));//服务器向客户端发送数据报
...
{
	m_ClientSock.Close();
	m_SrvSock.Accept(m_ClientSock);//服务器端与客户端建立连接
	m_IsConnect = TRUE;
	CSrvFiveChessDlg * pDlg = (CSrvFiveChessDlg*)GetParent();
	...
	CString csClientIP;
	UINT nPort;
	m_ClientSock.GetSockName(csClientIP,nPort);//获取客户端的IP和端口
	if (pDlg->m_RightPanel.m_UserList.GetItemCount()<2)//用户数小于2，
	{
		int nItem = pDlg->m_RightPanel.m_UserList.InsertItem(1,"");//插入一行
		pDlg->m_RightPanel.m_UserList.SetItemText(nItem,1,csClientIP);//客户端IP
		pDlg->m_RightPanel.m_UserList.SetItemText(nItem,2,csFormat);//客户端联机时间
	}
	else
	{
...
{
	if (m_IsConnect)//如果客户端已连接
	{
		TCP_PACKAGE tcpPackage;
		tcpPackage.cmdType = CT_NETTEST;  //网络测试
...
void CChessBorad::OnMenuSvrSetting()
{
	...
	{
		if (SrvDlg.DoModal()==IDOK)//显示对话框
		{
			UINT port = SrvDlg.m_Port;
			/*MessageBox(SrvDlg.m_HostName);
			MessageBox(SrvDlg.m_HostIP);*/
			CSrvFiveChessDlg * pDlg = (CSrvFiveChessDlg*)GetParent();
